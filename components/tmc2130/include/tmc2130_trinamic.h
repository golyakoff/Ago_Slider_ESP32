...
#ifndef TMC_IC_TMC2130_TRINAMIC_H_
#define TMC_IC_TMC2130_TRINAMIC_H_
...
// => TMC-API wrapper
extern esp_err_t tmc2130_readWriteSPI(uint16_t icID, uint8_t *data, size_t dataLength);
// => TMC-API wrapper
...
/**
 * @brief Read field value from register
 * @param icID Driver ID
 * @param field Field definition
 * @param value Pointer to store read value
 * @return ESP_OK on success, error code otherwise
 */
static inline esp_err_t tmc2130_fieldRead(uint16_t icID, TMC2130RegisterField field, uint32_t *value)
{
    if (value == NULL) {
    ...
    uint32_t reg_value;
    esp_err_t ret = tmc2130_readRegister(icID, field.address, &reg_value);
    if (ret != ESP_OK) {
        return ret;
    }
    
    *value = tmc2130_fieldExtract(reg_value, field);
    return ESP_OK;
}

/**
 * @brief Update field value in register data
 * @param data Raw register value
 * @param field Field definition
...
/**
 * @brief Write field value to register
 * @param icID Driver ID
 * @param field Field definition
 * @param value New field value
 * @return ESP_OK on success, error code otherwise
 */
static inline esp_err_t tmc2130_fieldWrite(uint16_t icID, TMC2130RegisterField field, uint32_t value)
{
    uint32_t reg_value;
    ...
    ret = tmc2130_readRegister(icID, field.address, &reg_value);
    if (ret != ESP_OK) {
        // If register is write-only, start from 0
        if (ret == ESP_ERR_NOT_FOUND || ret == ESP_ERR_INVALID_RESPONSE) {
            reg_value = 0;
        } else {
            return ret;
        }
    ...
    // Update field
    updated_value = tmc2130_fieldUpdate(reg_value, field, value);
    
    // Write back
    return tmc2130_writeRegister(icID, field.address, updated_value);
}
...
#endif
#endif
#endif /* TMC_IC_TMC2130_TRINAMIC_H_ */
