...
static esp_err_t readRegisterSPI(uint16_t icID, uint8_t address, uint32_t *value);
static esp_err_t writeRegisterSPI(uint16_t icID, uint8_t address, uint32_t value);

esp_err_t tmc2130_readRegister(uint16_t icID, uint8_t address, uint32_t *value)
{
    return readRegisterSPI(icID, address, value);
}

esp_err_t tmc2130_writeRegister(uint16_t icID, uint8_t address, uint32_t value)
{
    return writeRegisterSPI(icID, address, value);
}

/**
 * @brief Read TMC2130 register via SPI
 * @param icID Driver ID/chip select
 * @param address Register address (7-bit)
 * @param value Pointer to store read value
 * @return ESP_OK on success, error code otherwise
 */
esp_err_t readRegisterSPI(uint16_t icID, uint8_t address, uint32_t *value)
{
    uint8_t data[5] = { 0 };
    ...
    // First try to read from cache for write-only registers
    if (tmc2130_cache(icID, TMC2130_CACHE_READ, address, &cached_value)) {
        *value = (int32_t)cached_value;
        return ESP_OK;
    }
    
    // Clear write bit for read operation
    data[0] = address & TMC2130_ADDRESS_MASK;
    
    // Send the read request
    esp_err_t ret = tmc2130_readWriteSPI(icID, data, sizeof(data));
    if (ret != ESP_OK) {
        return ret;
    }
    
    // Small delay for SPI turnaround (optional, depends on your setup)
    // ets_delay_us(1);
    ...
    ret = tmc2130_readWriteSPI(icID, data, sizeof(data));
    if (ret != ESP_OK) {
        return ret;
    }
    
    // Combine bytes into 32-bit value
    *value = ((int32_t)data[1] << 24) | 
             ((int32_t)data[2] << 16) | 
             ((int32_t)data[3] <<  8) | 
             ((int32_t)data[4]);
...
/**
 * @brief Write TMC2130 register via SPI
 * @param icID Driver ID/chip select
 * @param address Register address (7-bit)
 * @param value Value to write
 * @return ESP_OK on success, error code otherwise
 */
esp_err_t writeRegisterSPI(uint16_t icID, uint8_t address, uint32_t value)
{
    uint8_t data[5] = { 0 };
    
    // Set write bit and address
    ...
    // Cache the register value for write-only registers
